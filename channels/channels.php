<?php
// telegram channels
$channels = [
	"ARv2ray" => ["vless"],
	"Awlix_ir" => ["vmess"],
	"azadi_az_inja_migzare" => ["vmess", "vless", "ss"],
	"BegzarProxy" => ["vless"],
	"BestV2rang" => ["vless"],
	"bright_vpn" => ["vless", "ss"],
	"Capital_NET" => ["vless"],
	"CloudCityy" => ["vless"],
	"config_proxy" => ["vless", "vmess"],
	"ConfigsHUB" => ["vmess", "vless", "trojan"],
	"crgaming7" => ["vless"],
	"customv2ray" => ["vmess", "vless"],
	"CUSTOMVPNSERVER" => ["vmess", "vless"],
	"DigiV2ray" => ["vless"],
	"DirectVPN" => ["trojan"],
	"FalconPolV2rayNG" => ["vless"],
	"flyv2ray" => ["vless"],
	"fnet00" => ["vmess", "vless", "trojan"],
	"FreakConfig" => ["vmess", "vless", "trojan", "ss"],
	"free1_vpn" => ["vmess", "vless", "vless"],
	"free4allVPN" => ["vmess", "ss"],
	"free_v2rayn1" => ["vmess", "vless"],
	"free_v2rayyy" => ["vmess", "vless"],
	"frev2ray" => ["vless"],
	"ghalagyann" => ["vless"],
	"God_CONFIG" => ["vless"],
	"Helix_Servers" => ["vless"],
	"Hope_Net" => ["vmess"],
	"info_2IT_channel" => ["vless"],
	"INIT1984" => ["vless"],
	"iP_CF" => ["vless", "vmess", "trojan", "ss"],
	"ipV2Ray" => ["vless"],
	"lranOnline_new" => ["vless"],
	"IRN_VPN" => ["vless"],
	"iSegaro" => ["vless"],
	"KAFING_2" => ["vless"],
	"Kurdistan_Vpn_Perfectt" => ["vless"],
	"L_AGVPN13" => ["vless"],
	"lightning6" => ["vless"],
	"Lockey_vpn" => ["vless"],
	"LoRd_uL4mo" => ["vmess", "vless"],
	"lrnbymaa" => ["vless"],
	"MehradLearn" => ["vless"],
	"melov2ray" => ["vless"],
	"MsV2ray" => ["vless"],
	"MTConfig" => ["vmess", "vless", "ss"],
	"NetFreedom0" => ["vmess", "vless", "ss"],
	"NETMelliAnti" => ["vless"],
	"NIM_VPN_ir" => ["vless"],
	"oneclickvpnkeys" => ["vmess", "trojan"],
	"Outline_ir" => ["vless"],
	"Outline_Vpn" => ["vless"],
	"Outlinev2rayNG" => ["vless"],
	"OutlineVpnOfficial" => ["ss"],
	"PAINB0Y" => ["vless"],
	"Parsashonam" => ["vless"],
	"polproxy" => ["vless"],
	"PrivateVPNs" => ["trojan"],
	"Proxy_PJ" => ["vmess", "vless"],
	"proxystore11" => ["vless", "vmess", "trojan"],
	"proxyymeliii" => ["vmess", "vless", "trojan", "ss"],
	"prrofile_purple" => ["vmess", "vless"],
	"rayvps" => ["vless"],
	"reality_daily" => ["vless"],
	"Royalping_ir" => ["vmess"],
	"rxv2ray" => ["vless"],
	"SafeNet_Server" => ["vmess", "vless"],
	"server444" => ["vless"],
	"serveriran11" => ["vless"],
	"ServerNett" => ["vless"],
	"ShadowProxy66" => ["vmess", "vless"],
	"ShadowSocks_s" => ["vless"],
	"shh_proxy" => ["vless"],
	"shopingv2ray" => ["vless"],
	"UnlimitedDev" => ["vless"],
	"v2_team" => ["vmess", "vless"],
	"v2Line" => ["vmess"],
	"V2parsin" => ["vmess", "vless"],
	"V2pedia" => ["vless"],
	"V2ray_Collector" => ["vless"],
	"v2ray_outlineir" => ["vmess"],
	"v2ray_swhil" => ["vmess", "vless", "trojan"],
	"v2ray_vpn_ir" => ["vmess", "vless", "trojan"],
	"v2ray1_ng" => ["vmess", "vless", "trojan"],
	"V2rayCollectorDonate" => ["vmess", "vless", "trojan", "ss"],
	"v2raying" => ["vmess", "vless", "ss"],
	"v2rayng_config_amin" => ["vless", "vmess"],
	"v2rayng3" => ["vless"],
	"V2rayng_Fast" => ["vmess"],
	"v2rayNG_Matsuri" => ["vless"],
	"v2rayng_vpnrog" => ["vless"],
	"V2rayNGmat" => ["vless"],
	"V2rayNGn" => ["vmess", "vless"],
	"V2rayngninja" => ["vless", "ss"],
	"v2rayngvpn" => ["vless"],
	"v2rayOpen" => ["vless", "vmess"],
	"V2RayTz" => ["vless", "trojan"],
	"v2trayproxy" => ["vless"],
	"vless_vmess" => ["vmess", "vless"],
	"VlessConfig" => ["vless"],
	"vmess_vless_v2rayng" => ["vmess", "vless", "trojan", "ss"],
	"vmesskhodam" => ["vmess", "vless"],
	"vmessorg" => ["vmess", "vless"],
	"VmessProtocol" => ["vless", "trojan", "ss"],
	"VPNCLOP" => ["vless"],
	"VPNCUSTOMIZE" => ["vless"],
	"VpnFreeSec" => ["vless"],
	"vpnmasi" => ["vless"],
	"VpnProSec" => ["vless"],
	"WebShecan" => ["vless"],
	"XsV2ray" => ["vless"],
	"yaney_01" => ["vmess", "trojan", "ss"],
	"zen_cloud" => ["vmess", "vless"],
];
